...
/* return count of specific file, for lock used, prevent umounted*/
...
int execute_tracer(int pid, char filename[]){
    ...
    char exec[NAME_MAX];
    char str[] = "/home/minyeon/syscall_hooking/tracer %d %s > /home/minyeon/syscall_hooking/tracer_out";
...
asmlinkage int my_openat(const struct pt_regs *regs)
{
	...
	if (error && (target[i] != NULL) && strstr(file_name, target[i]))
	{
	    ...
	    pr_info("[info] %s(pid %d)(state %d) -> %s(pid %d)(%d) -> %s(pid %d)(%d)-> %s %d %d -> %s %d %d-> %s %d %d\n",
